...
if (fs.existsSync(verifyRoute)) {
  ...
  source = source.replace(
    'if (!contact || !mobilesMatch(contact.Mobile, mobile)) {',
    `const normalizedInputMobile = String(mobile ?? "").replace(/\\D/g, "").slice(-10);\n    const normalizedZohoMobile = String(contact?.Mobile ?? "").replace(/\\D/g, "").slice(-10);\n    const emailMatches = String(contact?.Email ?? "").trim().toLowerCase() === String(email ?? "").trim().toLowerCase();\n    const mobileMatches = normalizedInputMobile.length === 10 && normalizedZohoMobile === normalizedInputMobile;\n    if (!contact || !emailMatches || !mobileMatches) {`,
  );
